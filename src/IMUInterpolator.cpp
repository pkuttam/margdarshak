...
margdarshak::IMUInterpolator::IMUInterpolator(margdarshak::FrameContainer& frameContainer, DatasetSaver* datasetSaver)
        : frameContainer(frameContainer), saver(datasetSaver)
{}

void margdarshak::IMUInterpolator::addAccData(vector<float> data, double timestamp)
{
    margdarshak::TimeMeasurement measurement("IMUInterpolator::addAccData");
...
void margdarshak::IMUInterpolator::insertAccDataIfNecessary()
{
...
void margdarshak::IMUInterpolator::addGyrData(vector<float> data, double timestamp)
{
    margdarshak::TimeMeasurement measurement("IMUInterpolator::addGyrData");
    std::unique_lock<std::mutex> lock(mutex);
...
void margdarshak::IMUInterpolator::insertGyrDataIfNecessary()
{
...
vector<float> margdarshak::interpolateData(const PartialIMUData& data1, const PartialIMUData& data2, double timestamp)
{
...
std::pair<std::vector<float>, margdarshak::IMUInterpolationResult>
margdarshak::interpolateDataFromArray(const vector<PartialIMUData>& array, double timestamp)
{
...
void margdarshak::IMUInterpolator::addImage(std::unique_ptr<margdarshak::ImageAndExposure> image, double timestamp)
{
    margdarshak::TimeMeasurement measurement("IMUInterpolator::addImage");
    std::unique_lock<std::mutex> lock(mutex);
...
void margdarshak::IMUInterpolator::trySendingImages()
{
...
margdarshak::PartialIMUData::PartialIMUData(
        const vector<float>& data,
        double timestamp) : data(data), timestamp(timestamp)
{}

bool margdarshak::PartialIMUData::operator<(const margdarshak::PartialIMUData& other) const
{
    return timestamp < other.timestamp;
}

margdarshak::IMUDataDuringInterpolation::IMUDataDuringInterpolation(double timestamp) : timestamp(timestamp)
{
    gyrSet = false;
    accSet = false;
}

bool margdarshak::IMUDataDuringInterpolation::operator<(const margdarshak::IMUDataDuringInterpolation& other) const
{
    return timestamp < other.timestamp;
}

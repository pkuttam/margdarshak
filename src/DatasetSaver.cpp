...
margdarshak::DatasetSaver::DatasetSaver(std::string saveFolder)
{
...
void margdarshak::DatasetSaver::saveImagesWorker()
{
    while(running)
    {
...
void margdarshak::DatasetSaver::addImage(cv::Mat mat, double timestamp, double exposure)
{
    {
        std::unique_lock<std::mutex> lock(mutex);
...
void margdarshak::DatasetSaver::addIMUData(double timestamp, std::vector<float> accData, std::vector<float> gyrData)
{
...
void margdarshak::DatasetSaver::end()
{
    running = false;
    frameArrivedCond.notify_all();
